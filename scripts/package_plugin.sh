...
rm -f -- "$repo/dist/jellyfin-plugin-jable-0.1.0.zip"
docker run --rm -v "$repo:/src" -w /src mcr.microsoft.com/dotnet/sdk:8.0 \
...
python3 -m zipfile -c dist/jellyfin-plugin-jable-0.1.0.zip \
    dist/plugin/Jellyfin.Plugin.Jable.dll dist/plugin/build.yaml
shasum -a 256 dist/jellyfin-plugin-jable-0.1.0.zip
