...
archive=${1:-"$repo/dist/jellyfin-plugin-jable-0.1.0.zip"}
...
mkdir -p "$temporary/config/plugins/Jable_0.1.0.0" "$temporary/cache"
python3 - "$archive" "$temporary/config/plugins/Jable_0.1.0.0" <<'PY'
import sys, zipfile
...
def request(path, data=None, expected=200, timeout=5):
    headers = {'Content-Type': 'application/json',
               'Authorization': 'MediaBrowser Client="Jable Smoke", Device="Local", DeviceId="jable-smoke", Version="0.1.0"'}
    if token:
